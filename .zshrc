# Exports {{{
export GITHUB_USER="Yanpeng.Lin"
export PYTHON_BIN=/usr/local/bin
export PATH=${PYTHON_BIN}:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:${TEX_PATH} # Reorder PATH so local bin is first
...
# Makes listing tmux session easier
function tmuxls() {
  tmux list-sessions
}
# }}}
...
RPROMPT='${PR_GREEN}$(virtualenv_info)%{$reset_color%} ${PR_RED}$(get_python_version) %t TTY-%l%{$reset_color%}'
# }}}
